...
namespace myutilootor.src
{
    internal class RE
    {
		...
		internal static readonly Regex R__2EOL = new(__2EOL, RegexOptions.Compiled);
		internal static readonly Regex R__LN = new(@"^\s*(~~.*)?$", RegexOptions.Compiled);
		internal static readonly Regex R_Empty = new(@"^$", RegexOptions.Compiled);
		...
		internal static readonly Dictionary<string, Regex> getLeadIn = new() {
        ...
        // regex patterns for specific grammar elements, to pull out their parameters
        internal static readonly Dictionary<string, Regex> getParms = new() {
			...
			["RuleActionKeepN"] = new Regex(@"^Keep\s+(?<i>" + _I + @")$", RegexOptions.Compiled),
